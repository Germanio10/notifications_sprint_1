...
class ContentFilmService:
    ...
    async def get_content(self, notification: Notification) -> Content:
        ...
        header = '<!DOCTYPE html><html lang="ru"><head><meta charset="utf-8"><title>Добро пожаловать!</title></head><body>'
        footer = '</body></html>'
        if notification.event_type == 'registered':
            subject = 'Поздравляем с регистрацией!'
            text = (
                header
                + '<h1>Привет {{ name }}! Спасибо за регистрацию в нашем кинотеатре!</h1><p> {{ content }} </p>'
                + footer
            )
            content = Content(
                subject=subject,
