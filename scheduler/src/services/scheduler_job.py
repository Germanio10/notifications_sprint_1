...
class SchedulerJob:
    ...
    async def add_by_date(
        self, task_id: uuid.UUID, run_date: datetime.datetime, args: tuple
    ) -> job.Job:
        if self.scheduler.get_job(job_id=str(task_id)):
            self.scheduler.get_job(job_id=str(task_id)).remove()

        return self.scheduler.add_job(
            self.send_to_broker, 'date', run_date=run_date, args=args, id=str(task_id)
        )

    async def add_by_cron(
        self, task_id: uuid.UUID, cron: str, timezone: str, args: tuple
    ) -> job.Job:
        if self.scheduler.get_job(job_id=str(task_id)):
            self.scheduler.get_job(job_id=str(task_id)).remove()
