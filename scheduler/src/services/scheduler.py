...
from utils.fake_user import User

from db.abstract_repository import AbstractRepository


class Scheduler:
    def __init__(self, db: AbstractRepository, scheduler_job: SchedulerJob, user: User) -> None:
        self.db = db
            ...
            '$or': [
                {
                    '$and': [
                        {'scheduled': True},
                        {'scheduled_timestamp': {'$gt': current_timestamp}},
                    ]
                },
                {
                    '$and': [
                        {'scheduled': True},
                        {'cron': {'$type': 'string', '$exists': True, '$ne': ''}},
                    ]
                },
            ]
        }
    ...
    async def scheduled(self, scheduled_notify: ScheduledNotification) -> None:
        ...
        for timezone in timezones:
            ids_users_with_same_timezone = await self.get_user_with_timezone(
                timezone, users_timezones
            )
    ...
    async def scheduled_by_date(self, scheduled_by_date: ScheduledByDate) -> None:
        run_date = await self.get_run_date_local(
            scheduled_by_date.scheduled_timestamp, scheduled_by_date.timezone
        )
    ...
    async def scheduled_by_cron(self, scheduled_by_cron: ScheduledByCron) -> None:
        ...
        logger.info(
            'Scheduled notification {} added to scheduler with cron: {} for timezone {}'.format(
                scheduled_by_cron.notification_id,
                scheduled_by_cron.cron,
                scheduled_by_cron.timezone,
            )
        )
    ...
    async def get_user_with_timezone(self, timezone, users_timezones) -> list[uuid.UUID]:
        return [
            user['user_id']
            for user in filter(lambda x: x.get('timezone') == timezone, users_timezones)
        ]

    async def remove(self, message: dict) -> None:
        msg = QueueRemove(**json.loads(message.body))
